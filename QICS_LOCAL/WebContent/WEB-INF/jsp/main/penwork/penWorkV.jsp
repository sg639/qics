...
String V_POC_NO = request.getParameter("V_POC_NO") == null ? "" : request.getParameter("V_POC_NO");
...
$(document).ready(function() {
	...
	$('#my-button4').bind('click', function(e) {
		...
		alert($("#C_LV").val()+" : "+$("#C_PP_POD").html());
		
		if($("#C_LV").val()!=$("#C_PP_POD").html())
		{
			if(confirm("급지 등급과 검사 등급이 상이합니다!\n전송하시겠습니까?"))
			{	
				return true;
			}
		}
...
/*	CalvMap.put("ZZ","ZZ");
	...
	<%
	  if("10.207.1.37".equals(request.getRemoteAddr())) {
	%>
	  //alert("ngValue="+ngValue);  
	<%
	  }
	%>
...
// BG_PATH 존재시 이미지 미출력 기본 지정 폴더가있음 system.properties padRootPath.... sg639
...
//	checkDATA();
	try {
  	var str = $("#C_POC_NO01_POD").val();
  	if (str.indexOf("C")==0) {
  		$("#ORGANIZATION_ID").val("82");	
  	}else if (str.indexOf("S")==0) {
  		$("#ORGANIZATION_ID").val("80");	
  	} else {
  		$("#ORGANIZATION_ID").val(ngValue);
  	}
...
function changeCmst(){
	...
	if(!isNaN(dataMin) && !isNaN(dataMax) ){
		for(var i=1;i<=18;i++) {  
			...
			//2016.09.20 조영대 : 측정치두께항목에 . 만 있는경우 . 지우기
  		if(value==".") {
  		  value = "";
  		  $("#"+id).val("");
  		  $("#"+id).removeClass("text-danger");
...
function checkDATA(){
		...
		if(cBest =="B") {
			for(var j=0; j<2;j++){
				for(var i =1; i <(cCoilStCount+1); i++){
					...
					//alert("B   i = "+i+"  cCoilStCount =  "+cCoilStCount+"    cBest = S.    tempMstart = "+tempMstart+" tempMend =  "+tempMend);
			...
			for(var i =1; i <(cCoilStCount+1); i++){
				...
				var tempMstart = $("#C_COIL_ST"+tempId).val();
				var tempMend= $("#C_COIL_ED"+tempId).val();
				//alert("R   i = "+i+"  cCoilStCount =  "+cCoilStCount+"    cBest = S.    tempMstart = "+tempMstart+" tempMend =  "+tempMend);
				if(tempMstart != "" && tempMend !=""){
					  var paramObject = new Object();
					  paramObject.COLLECT_TYPE = cBest;
					  paramObject.M_START = tempMstart;
			...
			for(var i =1; i <5; i++){
				...
				var tempMstart = $("#C_COIL_ST"+tempId).val();
				var tempMend= $("#C_COIL_ED"+tempId).val();
				//alert("S  i = "+i+"  cCoilStCount =  "+cCoilStCount+"    cBest = S.    tempMstart = "+tempMstart+" tempMend =  "+tempMend);
				if(tempMstart != "" && tempMend !=""){
					  var paramObject = new Object();
					  paramObject.COLLECT_TYPE = cBest;
					  paramObject.M_START = tempMstart;
	...
	//2016.12.07 주 결함코드 무조건 대문자처리. -조영대
	try {
	    if($("#C_MDCODE").val()!=""){
        $('#C_MDCODE').val( $.trim($('#C_MDCODE').val().toUpperCase())  );
			}
	...
	//2016.11.10 주 결함코드 무조건 대문자처리. -조영대
	try {
	    if($("#NG_CODE").val()!=""){
        $('#NG_CODE').val( $.trim($('#NG_CODE').val().toUpperCase())  );
			}
	...
	<%
	  if("10.207.1.37".equals(request.getRemoteAddr())) {
	%>
	  //alert("C_MDCODE="+$("#C_MDCODE").val());  
	  //alert("NG_CODE="+$("#NG_CODE").val());  
	<%
	  }
	%>
...
//	var isCheck = false;
	...
	for(var i=1; i<=cMspCount; i++){
		...
		if(cmstValue==".") {
		  cmstValue = "";
		  $("#C_MST"+tempId).val("");
		  $("#C_MST"+tempId).removeClass("text-danger");
		}
		...
		if($("#C_LV").val() !=""){
			...
			if(cLv =="T" || cLv =="H1" || cLv =="D2" || cLv =="D3" || cLv =="E2" || cLv =="E3" || cLv =="S" || cLv =="XX"
				...
				if(cLv =="T" || cLv =="H1"){
					 ...
					 /****************************************
					 * 임성현 수정부(2016-07-12)
					 * 수정내용 : 전기문 과장 요청으로 주결함 코드 체크로직 제외
					 ****************************************/
					 //if($("#C_MDCODE").val()!=""){
...
function sendErp(){ //데이터 확인 오류 및 수정
	...
	var status = result.DATA.Status;
			...
			if (status == "T") {
				...
				alert(message);
				parent.erpEnd($("#SEQ_Q100").val(), $("#SEQ_T300").val(), $(
						"#IN_LINE").val());
			} else if (status == "E") {
				message = result.DATA.Message;
				alert(message);
	...
	<form id="frameForm" name="frameForm" class="form-horizontal">
		...
		<div class="wrapPaper">
			...
			<div class="btn-wrapper">
				<div class="row">
					<div class="col-xs-7">
						<button id="my-button1" type="button"
							class="btn btn-success pull-left m-r-xs">
							<i class="fa fa-sticky-note-o"></i> 원본보기
						</button>
						<button id="my-button2" type="button"
							class="btn btn-danger pull-left m-r-xs">
							<i class="fa fa-exclamation-triangle"></i><span id="reportNm"></span>
						</button>
						<button id="my-button7" type="button"
							class="btn btn-danger pull-left m-r-xs">품질이상보고서 생성</button>
						<button id="my-button3" type="button"
							class="btn btn-default pull-left">분할양식추가</button>
					</div>
					<div class="col-xs-5">
						<button id="my-button4" type="button"
							class="btn btn-primary pull-right m-r-xs">
							<i class="fa fa-edit"></i> 내용저장
						</button>
						<button id="my-button5" type="button"
							class="btn btn-default pull-right m-r-xs disabled">ERP전송</button>
						<!--  <button type="button" class="btn btn-default pull-right m-r-xs" onclick="javascipt:cMdata();">ERPTEST</button>-->
						<button id="my-button6" type="button"
							class="btn btn-default pull-right m-r-xs"
							onclick="javascipt:deleteData();">삭제</button>
