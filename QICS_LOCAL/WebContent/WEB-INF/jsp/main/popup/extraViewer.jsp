...
.order_create_area .txt{display:block;padding-top:100px;text-align:center;color:#666;font-size:18px;line-height:26px;font-weight:bold}
  ...
  <title>품질검사정보수집시스템(QICS)-자료보기 :: ${param.POC_NO}</title>
	...
	if(result && result.DATA && result.DATA.length > 0 ){
		...
		for(var i = 0; i<result.DATA.length; i++){
            ...
            if("R"==result.DATA[i].STATUS_CODE) { //출력대기
            	 satusIcon ='<img src="/common/img/btn_p_1.png">';
          	} else if ("P"==result.DATA[i].STATUS_CODE) { //검사대기
          		satusIcon ='<img src="/common/img/btn_p_3.png">';
          		
          	} else if ("S"==result.DATA[i].STATUS_CODE) { //검사중
          		satusIcon ='<img src="/common/img/btn_p_4.png">';
          		
          	} else if ("C"==result.DATA[i].STATUS_CODE) { //전송완료
          		satusIcon = '<img src="/common/img/btn_p_5.png">';
          		
          	} else if ("ERP 전송실패"==result.DATA[i].STATUS_CODE) {
          		satusIcon ='<img src="/common/img/btn_p_6.png">';
          	} else {
			...
			if(i==0) {
				...
				sheet_list += '<div><h3 id="sheet_nav_'+slideIndex+'" class="slide-ON" onclick="javascript:changeSheet(\''+slideIndex+'\',\''+result.DATA[i].IN_LINE+'\',\''+result.DATA[i].SEQ_Q100+'\',\''+result.DATA[i].SEQ_T300+'\',\''+result.DATA[i].POC_NO+'\');">'+slideIndex+'. '+result.DATA[i].IN_LINE+', '+result.DATA[i].WORK_DATE+'&nbsp;'+errIcon+'<br>'+satusIcon+'( '+result.DATA[i].POC_NO02+' )</h3></div>\n';
				...
				sheet_list += '<div><h3 id="sheet_nav_'+slideIndex+'" onclick="javascript:changeSheet(\''+slideIndex+'\',\''+result.DATA[i].IN_LINE+'\',\''+result.DATA[i].SEQ_Q100+'\',\''+result.DATA[i].SEQ_T300+'\',\''+result.DATA[i].POC_NO+'\');">'+slideIndex+'. '+result.DATA[i].IN_LINE+', '+result.DATA[i].WORK_DATE+'&nbsp;'+errIcon+'<br>'+satusIcon+'( '+result.DATA[i].POC_NO02+' )</h3></div>\n';
			}
		...
		$(".slide-ON").removeClass("slide-ON");
	 	...
	 	try {
			...
			$("#extraViewerTitle").text("품질검사결과표 상세보기 ( "+ (selectedPOCNO!=""?selectedPOCNO:init_POC_NO) +" )");
		} catch (e) {
			// TODO: handle exception
		}
...
function changeSheet(p_slideIndex,p_IN_LINE,p_SEQ_Q100,p_SEQ_T300,p_POC_NO) {
	...
	$("#lodingTxtDiv").show().fadeIn('fast');
	 ...
	 try {
			//alert(top.document.getElementById("extraViewerTitle"));
			$("#extraViewerTitle").text("품질검사결과표 상세보기 ( "+ p_POC_NO +" )");
	} catch (e) {
			// TODO: handle exception
	}
...
function changeSheetDetail(p_SEQ_Q100){
	...
	if(statusCode =="R"){ //출력대기
		...
		url= "/SheetDetailView.do?FRM_DAT_ID="+frmDatId+"&SEQ_Q100="+li_idx+"&SEQ_T300="+seqT300+"&WEB_DATA_YN="+webDataYn+"&FORM_TYPE="+fTypeMap.get(inLine)+"&IN_LINE="+inLine+"&E_STATUS="+eStatus+"&STATUS_CODE="+statusCode+"&ERP_UPLOAD_YN="+erpUploadYN;
	}else{ // P 검사대기중.
		//alert("검사대기중입니다.");
		window.frames["basePenWork"].location.href="/Guide.do";
	}
        ...
        <nav class="navbar navbar-static-top white-bg" role="navigation">            
        	<div class="navbar-header">
                ...
                <div class="navbar-search">  
                	<h3 id="extraViewerTitle">품질검사결과표 상세보기</h3>
                		
                        <!--  <form role="form" class="form-inline">
                    	<div class="form-group">
                        	<label class="control-label">POC NO</label>
                			<input id="I_POC_NO" type="text" class="form-control width240px" style="font-size: 20px;font-weight: bold;ime-mode:disabled;text-transform:uppercase;" onKeyUp="this.value=this.value.toUpperCase();" value="${POC_NO}" placeholder="POC번호를 입력하세요.">
                        </div>
