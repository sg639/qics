	...
	function VFGetFileSize(curVF, callbackFileuploadObj, vfIndex) {
		...
		var options = {
			...
			error : function(jqXHR, textStatus, errorThrown) {
				if (errorThrown == "abort"){$.ajax(optionGetSize);}
			}
		};
		...
		var optionGetSize = {
			...
			success : function(data) {
				...
				sheetUpload_AddFile($("#uploadForm>#fileSeq").val(),infilename,retJObj.filesize );
...
//			url : FILEUPLOAD.CONTEXT+"/fileuploadJFileUpload.do?cmd=upload",
			success : function(data) {
				...
				//var  rowdata =$("#list1").getRowData(selRow);// 선택된 로우의 정보를 가져온다
				var ids = $("#list1").getDataIDs();
				alert(ids.length);
				alert(JSON.stringify(dataObj));
			//	rowdata.sFileNm = dataObj.filenameReal;
			//	rowdata.filePath = dataObj.filenamePath;
				$("#list1").setCell(ids,'sFileNm',	dataObj.filenameReal,'');
				$("#list1").setCell(ids,'filePath',	dataObj.filenamePath,'');
				//alert(JSON.stringify(rowdata));
			//	$("#list1").jqGrid('setRowData',ids,rowdata);
...
*/
	...
	function sheetUpload_AddFile(FileSeq,FileName, FileSize,FworkDir){
		...
		try{
			...
			var selDatas = {sNo:'',sDelete:'',sStatus:'I',enterCd:'',fileSeq:FileSeq,seqNo:'',rFileNm:FileName,sFileNm:'',filePath:'',fileSize:FileSize,chkdate:'',chkId:'',download:''};
			var selRow = 	$("#list1").getGridParam("selrow");

			var ids = $("#list1").getDataIDs();
			if(selRow == null){
				$("#list1").addRowData(ids.length+1,selDatas,'first');
			}else{
				$("#list1").addRowData(ids.length+1,selDatas,'after',selRow);
			}
