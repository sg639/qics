...
function getUser(){
	...
	var user = ajaxCall("/Employee.do?cmd=baseEmployeeDetail",$("#empForm").serialize(),false);
	if(user.map != null && user.map != "undefine") user = user.map;

	$("#userFace").attr("src","/ImageDownload.do?entercd=PTCH&sabun="+user.sabun);

	$("#searchKeyword"       ).val(user.name);
	$("#tdSabun"       ).html(user.sabun);
	$("#tdResNo"       ).html(user.resNo.substr(0,6)+"-*******");
	$("#tdOrgNm"       ).html(user.orgNm);
	$("#tdEmpYmd"       ).html(user.empYmd);
	$("#tdRetYmd"       ).html(user.retYmd);
	$("#searchRegNo_"	).val(user.resNo);
