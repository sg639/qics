 ...
 * 화  일  설  명	: 
 *
 * 저 작 권 공 지	: 이 소프트웨어는 대한생명보험주식회사 소유입니다.
 *            	    대한생명보험주식회사의 허가없는 배포 및 수정할 수 없습니다.
 * 				  Copyright 2006 by KOREALIFE INSURANCE Co., Ltd. AllRights Reserved. 
 * 
 * 패키지 버젼 	: 1.0
 * 
 * 작   성    자  	: 이한일
...
public class StringUtil {
    ...
    public static String getIncoding(String in){
    	  ...
    	  for (int i=0 ;i < in.length(); i++) {
    	   tmp = (int)(in.charAt(i) + key.charAt(i%20)%key.length()) +25288/key.charAt(i%20);
    	   
    	   if((tmp/10000) > 0) tmpCheck = ((int)key.charAt(i%20)/10 + 2)%10;
    	   else if((tmp/100) > 0) tmpCheck = ((int)key.charAt(i%20)/10 + 1)%10;
	 ...
	 */ 
    	 public static String getDecoding(String in){
    	  ...
    	  do {
    	   ...
    	   else if(tmpCheck < 2) tmpCheck = (tmpCheck+10) - (int)key.charAt(k%20)/10;
    	   else tmpCheck = tmpCheck - (int)key.charAt(k%20)/10;
